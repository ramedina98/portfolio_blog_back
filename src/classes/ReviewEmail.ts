...
class ReviewEmail implements EmailBaseMethods {
    ...
    constructor(name: string, email: string, tzClient: string, id_email: string, article_title?: string, article_link?: string, article_image?: string){
        this.name = name;
        this.email = email;
    ...
    async send(): Promise<void> {
        ...
        try {
            const [userResponse, adminResponse]: SentMessageInfo[] = await Promise.all([
                ...
                sendEmail(SERVER.EMAIL, subjectNotification, htmlTemplateNotification)
            ]);
